...
| Report Section   | `DiagnosticReport.text` | See [IMR DiagnosticReport](StructureDefinition-imr-diagnosticreport.html) for details | e.g. Findings and Impressions. See Note 2 |
{: .grid}
...
> Note 2: The Sender may consider using the experimental [IMR Observation](StructureDefinition-imr-observation.html) resource to encode contents such as findings and impressions in various report sections. A code may be used to identify what *section* the observation belongs to. For example, LOINC code __59776-5__ represents a procedure finding and LOINC code __19005-8__ represents a narrative impression. The Sender may consider encoding a single finding or impression per IMR Observation, or encoding all findings as a single string and all impressions as a single string to bridge existing applications. Furthermore, the Sender may consider using IMRObservation.hasMember to specify relationships between observations, e.g. linking impression to finding, or finding to another finding.
...
###### 2:4.141.4.1.2.2.2 Imaging Context in an IMR DiagnosticReport Resource
...
| `data-imaging-selection` | String | R | Reference to an `ImagingSelection` resource |
{: .grid}
...
> This functionality is necessary in case the Sender stores the report to the Report Repository with a URL that is inaccessible to other subsequent consumers of the reports.

The Report Repository may extract the embedded rendered report in DiagnosticReport.presentedForm.data, store them and substitute the corresponding DiagnosticReport.presentedForm with a URL (i.e., DiagnosticReport.presentedForm.url) instead.
